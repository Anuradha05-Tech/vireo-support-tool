...
def run_validation(sample_size: int = 40, random_seed: int = 42):
    ...
    report_lines = [
        ...
        f"A random sample of {len(sample_df)} flagged repeat-contact ticket pairs (Strategy 2: Category + SKU match) was hand-verified for semantic issue alignment.",
        f"The structural 30-day join achieved a **{precision_pct:.2f}% precision rate** ({tp_count} true positives out of {len(sample_df)} pairs).",
        "",
